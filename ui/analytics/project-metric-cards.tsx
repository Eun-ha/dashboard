import {
  fetchMetricsData,
  fetchTopPathnames,
} from "@/backend/project-metric-data";
...
export async function MetricCardHolder({
  session,
  project = null,
}: {
...
export async function VitalCard({
  ...
  const processedChartData = metricInfo.ms2s
    ? chartData.map((data) => {
        return {
          ...data,
  ...
  const processedBarData = metricInfo.ms2s
    ? barData.map((data) => {
        return {
          ...data,
          value: Math.round((data.value / 1000) * 100) / 100,
